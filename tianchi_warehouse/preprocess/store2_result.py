...
def writeResult(result):
    w = open("../data/20160514result/store2_result.csv", 'ab')
    write = csv.writer(w)
    write.writerow(result)
    w.close()

def store2Result():
    f = open("../data/trainset2/2_two_week.csv")
    rows = csv.reader(f)
    for row in rows:
        data = []
        # arrs = []
...
# def store2Result():
#     f = open("../data/trainset/store2_two_week.csv")
#     rows = csv.reader(f)
#     for row in rows:
#
#         data = []
#         value = 0.709 + 0.425 * int(row[2]) - 0.057 * int(row[3]) - 0.008 * int(row[4]) + 0.481 * int(row[5]) - 0.143 * int(row[6])
#         value = round(value, 1)
#         data.append(row[0])
#         data.append("2")
#         data.append(value)
