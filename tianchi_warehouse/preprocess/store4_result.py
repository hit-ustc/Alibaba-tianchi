__author__ = 'CC'
import csv



def writeResult(result):
    w = open("../data/20160514result/store4_result.csv", 'ab')
    write = csv.writer(w)
    write.writerow(result)
    w.close()

def store4Result():
    f = open("../data/trainset2/4_two_week.csv")
    rows = csv.reader(f)
    for row in rows:
        data = []
        # arrs = []
        # arrs.append(int(row[1]))
        # arrs.append(int(row[2]))
        # arrs.append(int(row[3]))
        # arrs.append(int(row[4]))
        # arrs.append(int(row[5]))
        # arrs.append(int(row[6]))
        # arrs.append(int(row[7]))
        # arrs.append(int(row[8]))
        # arrs.sort()

        temp = (int(row[1]) - int(row[7]))/6.0
        temp = round(temp, 1)
        data.append(row[0])
        data.append("4")
        data.append(int(row[1]) + temp)
        writeResult(data)

# def store4Result():
#     f = open("../data/trainset/store4_two_week.csv")
#     rows = csv.reader(f)
#     for row in rows:
#
#         data = []
#         value = 3.136 + 0.234 * int(row[2]) + 0.383 * int(row[3]) - 0.017 * int(row[4]) + 0.363 * int(row[5]) - 0.190 * int(row[6])
#         value = round(value, 1)
#         data.append(row[0])
#         data.append("4")
#         data.append(value)
#         writeResult(data)

store4Result()

